...
if [[ -f "$VERIFY_SCRIPT" ]] && command -v python >/dev/null 2>&1; then
  ...
  for required in SKILL.md references/iterative-controller.md references/multi-route-controller.md references/report-contract.md scripts/init_annotation_project.py scripts/check_completion_gate.py scripts/request_master_quality_review.py scripts/record_master_quality_approval.py scripts/build_report.py; do
