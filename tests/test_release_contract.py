...
class SheepOvaryReleaseContract(unittest.TestCase):
    def test_profile_has_rfirst_fixed_parameters_and_atlas_policy(self) -> None:
        ...
        self.assertEqual(profile["external_reference_policy"]["atlas_mapping_scope"], "complete_analysis_set_once_after_merge_and_freeze_broad")
        self.assertEqual(profile["external_reference_policy"]["atlas_direct_writeback_scope"], "unlabeled_post_merge_observations_only")
    ...
    def test_seurat_resolution_grids_expose_real_parallel_workers(self) -> None:
        ...
        self.assertIn("cluster_one_resolution", cohort)
        self.assertIn("resolution_seeds", cohort)
        self.assertIn("future.apply::future_lapply", cohort)
        self.assertIn('object=q[["COHORT_snn"]], algorithm=4', cohort)
        self.assertNotIn('resolution=resolutions,', cohort)
