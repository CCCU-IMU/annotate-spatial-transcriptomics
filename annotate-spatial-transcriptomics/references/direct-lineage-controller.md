...
2. **Initial broad decisions.** Review the frozen matrix against the open-world catalog. First validate that every default lineage exposes at least two explicit positive marker families. Assign a moderate-or-higher `initial_broad_label` when absolute full-feature detection/pseudobulk supports its backbone and support family, alternatives/anti-programs are cleared, the winner margin is positive and no material contradiction remains. Centered module score and one-vs-rest DEG cannot by themselves reject a tissue-wide parent whose markers are shared across several clusters. Only genuinely low-information, featureless or irreducibly mixed observations enter `qc_holdout`.
