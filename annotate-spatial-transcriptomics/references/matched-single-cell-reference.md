...
- Map detailed reference labels to a shallow candidate vocabulary before transfer. Many single-cell immune labels may map to `Immune`; ordinary endothelial labels challenge `Endothelial`, pericyte/mural/perivascular labels challenge independent `Pericyte/mural`, and lymphatic labels challenge a parent-locked `Lymphatic endothelial` identity. A low-resolution `Vascular-associated` reference label has no direct transfer target.
- Keep evidence-dependent lineages such as `Theca`, `Pericyte/mural`, `Smooth muscle`, `Neural/Schwann` and strict `Oocyte` behind their tissue-profile gates even when the reference contains those names. Reference proximity or ACTA2/TAGLN cannot establish mural or smooth-muscle identity.
...
- Do not merge reference cells into the spatial query for final DEG, detection percentages or marker dotplots. Public DEG/dotplots use every spatial observation in the single `final_cell_type`; broad/fine fields remain internal audit provenance.
...
For sheep ovary, a count-level, stage-compatible matched reference is the preferred external channel only after every initial-cluster cohort/local split has merged and formal broad membership is frozen. Run one broad-only mapping over the complete analysis set. Unlabeled post-merge observations are eligible for calibrated broad-only rescue; defined broad/fine cells use the same mapping only for concordance challenge. A dotplot-only artifact remains marker evidence and cannot support all-cell mapping. Otherwise GSE233801 split-wall v2 is the primary public adult-sheep somatic Atlas. It supports independently reconstructed Granulosa, Stromal/mesenchymal, Endothelial, Pericyte/mural, Smooth muscle and Immune broad prototypes; Epithelial/mesothelial and Theca remain challenge-only, and it cannot rescue Oocyte or Luteal.
...
Display the matched single-cell dotplot as a reference panel, not as the spatial result. Final spatial DEG and canonical/data-specific marker dotplots must be recomputed from the complete accepted `final_cell_type` memberships.
