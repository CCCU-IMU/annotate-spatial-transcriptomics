...
- `route_attempt_registry.tsv`: RCTD/Atlas assistance. The terminal Atlas row uses `source_state=residual_qc_holdout_after_all_cohorts`, binds the complete final QC membership/hash directly, has no QC-anchor prerequisite, records calibration/reference/channel evidence, and partitions the query into accepted broad-only versus QC reject outcomes. RCTD rows retain tier and QC-successor provenance.
...
- `provenance/annotation_membership_partition_audit.json`: exact, disjoint closure of initial broad cohorts, subcluster outcomes, direct returns, targeted successors, terminal residual QC, Atlas accepted/rejected outcomes and the final analysis set.
...
Every closed biological or interface decision requires `closure_rationale` and a resolvable `validation_artifact`. Every terminal context-specific identity requires `validation_status=passed` plus its validation artifact. Every supported initial broad class has one terminal broad-class cohort unless an underpowered skip is recorded. Atlas receives the complete frozen residual QC membership directly; there is no mandatory QC reclustering. Every interface requires a deconvolution applicability record only when RCTD is attempted. Scheduler failures use terminal status `failed_preserved`; a repaired execution receives a new run ID. Cohort IDs are versioned and membership hashes are immutable.
