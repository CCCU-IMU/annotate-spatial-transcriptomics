...
Atlas rescue was broad-only. The external Atlas, current-query marker/anti-marker channel and an internal-anchor or observed-density spatial channel had to agree under thresholds calibrated on disjoint query-like held-out current-query anchors. High and moderate-only returns entered the inclusive broad membership with `fine_anchor_eligible=false`; low calls remained review/QC. A reference self-split was never treated as query calibration.

## 4. Rare and easily confused boundaries

- Oocyte required multiple non-zona identity/maternal-ooplasm genes, somatic anti-program clearance, compact follicular object morphology and strict candidate reclustering. Zona transcripts in otherwise coherent Granulosa were treated as ambient/adjacent signal.
...
The frozen release maintained strict, inclusive and display views. The primary broad DEG and both broad canonical/data-specific tree dotplots used every inclusive biological observation, including Atlas/anchor-rescued broad-only observations. Strict broad results were retained as sensitivity evidence. Fine DEG/dotplots used only strict observations with real fine labels.

Entry-QC exclusions, interfaces, remaining QC holdout and technical states were fully accounted for but excluded from the biological broad census and DEG. The final Chinese HTML contained the annotated whole-section map, expandable broad/fine tree, per-node spatial highlights, broad and subtype tree dotplots, marker spatial maps grouped by supported cell type, route/threshold/outcome panels, a Chinese event timeline, raw state records and checksums. Final assets were generated only after the master/user confirmation bound the frozen ledger and completion gate.
