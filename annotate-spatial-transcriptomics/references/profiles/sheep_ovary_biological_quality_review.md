...
- Resolve specific wall identities before the generic Stromal remainder. Compare directly supported Theca, Vascular-associated and mature Smooth-muscle identities; preserve unresolved specific-specific ties; only observations with no specific identity return to Stromal. Ovarian stroma is a broad background rather than a compulsory thin ring, so require that it extends beyond the contractile wall in the ROI, not that its global median radius exceed every Smooth-muscle observation.
