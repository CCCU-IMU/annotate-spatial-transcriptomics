...
This route is available only after second-round cohort scoring identifies one exact Leiden subcluster with either two independently supported specific identity cores, or one orthogonally supported specific identity embedded in a supported generic remainder. Whole-subcluster DEG is supporting evidence, not a trigger requirement: a minority Granulosa, Smooth muscle, Endothelial, Pericyte/mural or other lineage can have negative aggregate DEG after dilution by the majority parent. The minority seed must contain a candidate-declared direct discriminator, and at least one neighboring higher-resolution Leiden partition must reproduce the proposed identities as enriched, separable groups. Only then do candidate-local expression/spatial components define the exact selected-subcluster member set. Shared programs and exclusive tails not reproduced by reclustering remain watches. The route must never run as a whole-tissue or whole-cohort per-cellbin classifier.
