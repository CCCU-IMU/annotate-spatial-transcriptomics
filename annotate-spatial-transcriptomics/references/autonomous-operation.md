...
10. Stop only when the canonical `materialize_final_release` manifest, final-cell-type assets and `release_audit.json` are all PASS. `autopilot_status.py` is only a scheduling aid; it cannot confer v2.2 release authority.
