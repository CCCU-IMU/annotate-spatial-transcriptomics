...
3. Generate the scheduler-visible name with `scripts/scheduler_job_name.py`. Before submission run `scripts/preflight_generated_job.py --scheduler-job-name EXACT_NAME --python EXACT_PYTHON --python-imports numpy,pandas,... --rscript EXACT_RSCRIPT --r-packages Seurat,Banksy,...` on the generated script and parameter/profile files in the same environments the job will use; syntax, name, dependency or profile/grid failures stop before `csub`. On HNAICC submit an AIP through standard input (`csub < job.aip`), never as a positional path. Append the run to `state/run_registry.tsv` with status `submitted`, job ID, environment and logs. Supply a deterministic `work_key`, execution fingerprint, owner assignment and positive attempt. Only one `prepared/submitted/running` run may own a work key.
...
The completion audit is the one exception to pre-registration: do not insert its own run as `submitted`, because the gate correctly rejects every nonterminal registered run. Run the audit against the terminal registry, then append the audit run only after the scheduler has reached a terminal state. This avoids a circular self-registration deadlock without hiding any execution.
