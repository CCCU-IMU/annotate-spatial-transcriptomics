...
Use local execution for discovery and small audits. For submitted analysis read `references/job-orchestration.md` and follow submit â†’ monitor â†’ log inspection â†’ repair/resubmit â†’ artifact validation â†’ state writeback. Use the site's scheduler for memory-heavy expression operations. Scheduler templates are examples only; detect the current environment and available resources.
