...
9. Compare every mapped observation with the frozen primary state using `build_global_atlas_concordance.py` and route it with `route_calibrated_atlas_by_primary_state.py`. A frozen-QC observation with no existing biological label returns broad-only when its calibrated Atlas tier is moderate-or-higher and it has no OOD, ontology conflict or profile-excluded Atlas class; set `fine_anchor_eligible=false`. Current-query marker/anti-marker and spatial evidence remain audit/challenge channels and may veto a coherent group after review, but are not a second per-cell prerequisite for this direct broad return. Defined broad labels are never overwritten by mapping alone: agreement closes, material disagreement triggers one cluster/cohort review, and weak mapping is logged without erasing the query label.
...
7. register every project-local normalized/SCT/full-feature validation object with raw-count and analysis-set ancestry; never read another annotation project's derived expression object as query evidence
8. for a tissue profile, run `validate_broad_marker_family_contract.py` before initial broad decisions and save `provenance/broad_marker_family_contract_validation.json`
...
Use `route_calibrated_atlas_by_primary_state.py` for the default all-cell Atlas result. Unlabeled frozen-QC moderate/high predictions return broad-only after OOD, ontology and profile-scope safety gates; defined observations are comparison-only. `adjudicate_multichannel_broad_rescue.py` remains available for a genuinely uncalibrated or non-Atlas multi-evidence proposal, but its marker-plus-spatial gate must not be layered on top of an already calibrated Atlas tier as a duplicate per-cell requirement. Every accepted return participates in final broad DEG/dotplots but cannot seed fine discovery. Precompute the Atlas feature transform, low-dimensional representation and approximate-neighbor index once and validate `atlas_index_manifest.schema.json` with `validate_atlas_index_manifest.py`; never form a full query-by-reference distance matrix or repeat joint Atlas integration per sample. Treat low margin, mixed neighbors and coherent OOD programs as unknown/review signals, not nearest-class assignments.
...
2. run `validate_prelabel_broad_evidence.py`, `validate_broad_marker_family_contract.py`, `validate_project_input_boundary.py`, `validate_lineage_signal_coverage.py`, `validate_global_atlas_concordance.py`, `validate_broad_class_completeness.py`, `validate_residual_qc_audit.py`, `audit_release_taxonomy.py`, `validate_direct_lineage_workflow.py`, `validate_annotation_support_registry.py` and `audit_annotation_membership_partition.py`;
