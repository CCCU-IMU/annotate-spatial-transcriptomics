...
Start one bounded observation-level separability check when a second-round subcluster contains either two competing specific identity cores, or one orthogonally supported specific identity embedded in a supported generic remainder. Do not require positive whole-subcluster DEG to start this check: a minority Granulosa, Smooth muscle or other lineage can be diluted by the majority background. Actual writeback still requires observation discriminators, candidate-local DEG/pseudobulk, spatial components or neighboring-resolution partitions to demonstrate a separable member set. If the signals remain coexpressed in the same observations and no component validates, do not split; retain the supported parent broad and keep the alternative as `watch`. A high contradiction fraction in the aggregate mixed subcluster must not erase split candidates; contradiction is evaluated again on each proposed local subset. Generate every candidate subset independently, resolve overlaps with normalized evidence and pairwise discriminators, and leave ambiguous members in a supported common parent or `unresolved_biological`. Specific lineages are resolved before a generic Stromal remainder.
...
Run one calibrated all-cell broad Atlas mapping only after broad freeze. Atlas may directly rescue an unlabeled, non-OOD, profile-compatible observation at moderate-or-higher confidence; it cannot silently overwrite an existing broad label or create a fine label. A material conflict reopens the complete source subcluster/cohort once for query-derived biological review.
...
After any contract-required tissue-specific biological review and bounded repair, run `audit_catalog_wide_lineage_challengers.py` across every context-evaluable broad lineage, including labels already present and labels currently absent. The **decision and reporting unit is one broad cell type**. For each annotated broad, one review simultaneously checks **precision** inside the current label and **recall** across all other query observations, then produces one retain-or-patch decision for that cell type. Original second-round source subclusters, direct multi-family spatial components and source-subcluster `group watch` records are child evidence and exact patch bounds, not separate user-facing review tasks. A group watch is raised when selected-resolution DEG/pseudobulk/multichannel evidence predicts more lineage members than the sparse observation scorer recovered; it authorizes targeted raw-count review only and cannot label the whole source group. Missing broad lineages are queued separately only when query evidence produces a challenger. Low-RNA members may inherit only inside a supported current-label group, and generic Stromal is not allowed to generate whole-section recall components or group watches. The audit writes no labels. Resolve each cell-type queue item with query raw-count marker families, target-versus-outside DEG/pseudobulk, pairwise competitors, whole-section spatial distribution, over-recall and under-recall; geometry alone has no writeback authority. At most two decision rounds may be applied, and every applied round must rerun the complete catalog-wide audit. A retained cell type is closed only for the exact reviewed cell-ID scope signature. Stable partitions are reused; rerun SCT/PCA/SNN/Leiden only when that cell type's existing source units are genuinely not evaluable. If this review changes membership, rerun completeness and the tissue-specific biological review on the revised full object.

In user-facing progress, this stage has exactly one name: **逐大类全样本复核** (`per-broad whole-query review`). Do not call it “目录复核”, “目录审阅”, “来源组复核”, “强制重放” or any near-synonym. Every queued broad must have one controller-built evidence packet binding all four conclusions: current-member precision, whole-query recall, raw-count/full-transcriptome molecular identity and whole-section spatial consistency. Free text, keyword presence or a project-local decision script cannot substitute for that packet. If a packet contains unresolved over-recall or under-recall challengers, the broad cannot be retained by merely writing that they were refuted; it requires an exact cell-ID patch or a bounded targeted review. After any patch, regenerate the evidence packet for every broad on the revised membership. For sheep ovary, bind the Oocyte canonical endpoint and follicle-ROI endpoint into the relevant broad packets before closure.

A present broad review is incomplete unless it explicitly records four conclusions for that one cell type: current-member precision, whole-query recall, molecular identity and whole-section spatial consistency. Source subclusters and components may justify or bound a patch, but they cannot close any of these four broad-level conclusions on their own.
...
Within a reopened follicle wall, distinguish layer detection from writeback: local/coherent signal may prove a layer exists, whereas directly discriminated identity cores or a validated expression subcluster authorize membership. Formal Theca interna is steroidogenic/androgenic; structural/perifollicular Theca remains exploratory. Resolve Theca, Vascular-associated and mature nonvascular Smooth muscle before the generic Stromal remainder, and never demand mutually exclusive labels for the same mixed cellbin.
...
Write the frozen membership back with `write_frozen_annotations_to_seurat.R`; analysis membership plus the optional excluded-initial-QC ledger must exactly cover the Seurat object. Every semantic repair or proposal overlay must first pass through `apply_cell_id_membership_patch.py`: proposal IDs are unique and contained in the frozen base, updated values are joined by `cell_id`, base order is restored and non-proposal members remain byte-equivalent in the updated columns. Positional row assignment is forbidden. Build the sample-agnostic HTML with `build_frozen_review_report.py`. Before user confirmation, use `pending_user_review`; after confirmation, use `approved_final` without changing membership. The report contains broad/fine census, spatial overviews, high-contrast fixed-point-size highlights, switchable normalized/absolute marker dotplots, label-independent all-cell marker maps, evidence summaries and unresolved regions.
...
- final: broad/fine census, spatial structure, retained QC/Unknown and regions needing review.
