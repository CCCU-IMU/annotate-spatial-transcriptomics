...
An ordinary fine/state candidate never reconstructs its broad parent during broad closure; it remains a parent-bound proposal for materialization after broad freeze. A fine candidate may propose broad reconstruction only when the catalog explicitly declares `parent_broad_reconstruction_allowed=true` for a biologically justified route. Even then, its exact members must pass the complete parent-broad release policy—including all parent required families, candidate-specific direct/DEG/discriminator rules, context eligibility and contradiction limits—in addition to the fine discriminator. A synthetic `parent_identity` field, overlapping parent/fine genes or a strong fine/state program cannot bypass a failed broad parent. If the parent test fails, reject the promotion and return the members to local remainder review; never expand the surrounding cluster. In particular, Early/Late/Transitional luteal programs cannot create Broad Luteal when the `luteal_steroidogenic` parent fails its direct-joint or query-local DEG rule.
...
- every context-evaluable broad receives exactly one post-Atlas full-query specialist review covering present-label precision and outside-label recall; later evidence-bounded transfers into or out of a closed broad do not requeue that review;
