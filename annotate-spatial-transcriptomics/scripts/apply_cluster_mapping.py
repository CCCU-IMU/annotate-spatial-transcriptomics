...
def main() -> int:
    ...
    fields = ["sample_id", "cell_id", "decision_id", "spatial_object_id", "analysis_scope", "input_snapshot_id", "source_method", "source_run_id", "source_cluster", "source_key", "parent_decision_id", "generation", "route", "route_attempt_id", "route_run_id", "iteration", "state_tags", "spatial_tags", "qc_tags", "candidate_lineages", "state", "initial_broad_label", "broad_label", "fine_label", "confidence", "evidence_status", "validation_status", "validation_artifact", "validation_feature_scope", "recluster_cohort_id", "assignment_mode", "cross_lineage_target", "fine_anchor_eligible", "decision_version", "supersedes", "closed", "closure_rationale", "next_action", "created_at"]
    ...
    with inp, out:
        ...
        for row in reader:
            ...
            writer.writerow({
                ...
                "source_key": m.get("source_key",row.get("source_key",f"{args.source_run}|{cl}")), "parent_decision_id": m.get("parent_decision_id",row.get("parent_decision_id","")), "generation": m.get("generation",row.get("generation",m.get("iteration","1"))), "route": m["route"], "route_attempt_id": m.get("route_attempt_id",row.get("route_attempt_id","")), "route_run_id": m.get("route_run_id", ""), "iteration": m.get("iteration", "1"), "state_tags": m.get("state_tags",row.get("state_tags","")), "spatial_tags": m.get("spatial_tags",row.get("spatial_tags","")), "qc_tags": m.get("qc_tags",row.get("qc_tags","")), "candidate_lineages": m.get("candidate_lineages",row.get("candidate_lineages","")), "state": m["state"], "initial_broad_label": m.get("initial_broad_label", m.get("broad_label", "")),
                "broad_label": m["broad_label"], "fine_label": m["fine_label"], "confidence": m["confidence"],
                "evidence_status": m["evidence_status"], "validation_status": m.get("validation_status", "not_required"), "validation_artifact": m.get("validation_artifact", ""), "validation_feature_scope": m.get("validation_feature_scope", "unknown"), "recluster_cohort_id": m.get("recluster_cohort_id", ""), "assignment_mode": m.get("assignment_mode", "initial_broad_direct"), "cross_lineage_target": m.get("cross_lineage_target", m.get("cross_branch_target", "")), "fine_anchor_eligible": m["fine_anchor_eligible"],
