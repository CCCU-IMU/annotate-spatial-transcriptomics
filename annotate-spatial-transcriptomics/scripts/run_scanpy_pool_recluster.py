#!/usr/bin/env python3
"""Query-only Scanpy pool reclustering in a joint query/anchor PCA space."""
...
def main() -> None:
    ...
    if ncomp < 2:
        raise RuntimeError("pool too small for PCA")
    ...
    for resolution in map(float, args.resolutions.split(",")):
        ...
        if coordinates:
            ...
            fig, ax = plt.subplots(figsize=(9, 7)); ax.scatter(xx, yy, c=np.array([lut[x] for x in clusters]), s=.25, cmap="turbo", rasterized=True); ax.invert_yaxis(); ax.set_aspect("equal"); ax.set_axis_off(); ax.set_title(f"Query-only pool spatial {key}"); fig.savefig(args.out / "figures" / f"{key}_spatial.png", dpi=360, bbox_inches="tight"); fig.savefig(args.out / "figures" / f"{key}_spatial.pdf", bbox_inches="tight"); plt.close(fig)
    ...
    query.write_h5ad(args.out / "pool_reclustered_query.h5ad", compression="gzip")
    manifest = {
