...
def main() -> int:
    ...
    for row in cells:
        ...
        if cell_type:
            final_type_counts[cell_type] += 1
        else:
            retained_counts[row.get("final_state", "") or "pending_review"] += 1
    if not final_type_counts:
        raise SystemExit("confirmation review has no final_cell_type labels")
    ...
    for row in support_rows:
        if row.get("label_level") == "broad" and row.get("broad_label"):
            final_support[row["broad_label"]] = row
        elif row.get("label_level") == "fine" and row.get("fine_label"):
            final_support[row["fine_label"]] = row
    biological_types = set(final_type_counts) - {"QC/Unknown"}
    missing_support = sorted(biological_types - set(final_support))
    if missing_support:
        raise SystemExit(
            f"validated final-cell-type support reasons are incomplete: {missing_support}"
        )
    ...
    if marker_spatial_required:
        ...
        expected_groups = biological_types
        ...
        n_analysis = sum(final_type_counts.values()) + sum(retained_counts.values())
    ...
    if asset_manifest.get("status") != "PASS" or asset_manifest.get("label_column") != "final_cell_type":
        raise SystemExit("review assets did not pass or were not built from final_cell_type")
    ...
    if set(palette) != set(final_type_counts):
        raise SystemExit("review palette labels do not exactly match final_cell_type")
    ...
    final_type_rows = []
    for label, count in final_type_counts.most_common():
        support = final_support.get(label, {})
        final_type_rows.append({
            "最终细胞类型": label, "数量": count, "置信度": support.get("confidence", ""),
    ...
    palette_html = "".join(
        f"<span class='chip'><i style='background:{html.escape(palette[label])}'></i>{html.escape(label)} ({final_type_counts[label]:,})</span>"
        for label in final_type_counts
    )
    quality_rows = []
...
<section class='warn'><b>确认目标：</b>唯一公开注释列 final_cell_type。请重点检查类型数量、支持/反证、空间分布及典型 marker；若需修改，返回迭代路线而不是直接确认。</section>
...
<section><h2>最终细胞类型配色与数量</h2>{palette_html}</section>
<section class='grid'><article><h2>final_cell_type 空间投影</h2><img src='{png_data_uri(assets['spatial_png'])}'></article><article><h2>final_cell_type 典型 marker dotplot</h2><img src='{png_data_uri(assets['dotplot_png'])}'></article></section>
<section><h2>典型 marker 全 cellbin 空间表达</h2><p>不按注释标签筛选；固定点径；颜色为每个基因独立记录的表达尺度。</p>{marker_spatial_html}</section>
<section><h2>最终细胞类型注释支持原因</h2>{table(final_type_rows, ['最终细胞类型','数量','置信度','正向 marker','anti-marker','分辨率/稳定性','空间依据','路线'])}</section>
<section><h2>保留的非生物学状态</h2>{table([{'状态': key, '数量': value} for key, value in retained_counts.most_common()], ['状态','数量'])}</section>
    ...
    manifest = {
        ...
        "final_cell_types": dict(final_type_counts),
