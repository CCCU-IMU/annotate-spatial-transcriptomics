...
def main() -> int:
    ...
    for level, title in [("broad", "大类 marker 带树点图"), ("subtype", "亚群 marker 带树点图")]:
        ...
        for r in dot:
            ...
            if not r.get("absolute_png") or not r.get("absolute_pdf"):
                raise SystemExit("dotplot index lacks absolute assets; rebuild with build_marker_dotplots.R")
            ...
            absolute_png = rel(str(asset_path(r["absolute_png"], root)), report_dir); absolute_pdf = rel(str(asset_path(r["absolute_pdf"], root)), report_dir)
            card_id = re.sub(r"[^A-Za-z0-9_-]+", "-", f"dot-{level}-{r.get('panel','')}")
            cards.append(
                f"<article class='card dot-card' id='{html.escape(card_id)}'><h3>{html.escape(r.get('panel',''))}</h3>"
                f"<p>view={html.escape(r.get('analysis_view','未记录'))}；cohort={html.escape(r.get('evidence_cohort','未记录'))}</p>"
                f"<p><button onclick=\"setDotView('{html.escape(card_id)}','normalized')\">基因内归一化</button>"
                f"<button onclick=\"setDotView('{html.escape(card_id)}','absolute')\">绝对值</button></p>"
                f"<img class='dot-normalized' src='../{html.escape(png)}'>"
                f"<img class='dot-absolute' src='../{html.escape(absolute_png)}' style='display:none'>"
                f"<p><a href='../{html.escape(pdf)}'>归一化 PDF</a> · "
                f"<a href='../{html.escape(absolute_pdf)}'>绝对值 PDF</a> · "
                f"<a href='../{html.escape(src)}'>源表</a></p></article>"
            )
        sections.append(f"<section id='dot-{level}'><h2>{title}</h2><p>同一位置可切换基因内归一化与绝对值视图；两者来自同一源表。绝对平均表达按全图 q99.5 截断，仅用于可视化。</p><div class='grid'>{''.join(cards) or '<p>缺失：发布审计将失败。</p>'}</div></section>")
