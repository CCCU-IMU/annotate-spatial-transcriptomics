...
dir.create(a$out,recursive=TRUE,showWarnings=FALSE);gene_dir<-file.path(a$out,"spatial_genes");dir.create(gene_dir,showWarnings=FALSE);dir.create(file.path(a$out,"tables"),showWarnings=FALSE);scope<-ifelse(is.null(a$scope),"all_analysis_set_observations",a$scope);if(scope!="all_analysis_set_observations")stop("--scope must be all_analysis_set_observations for confirmation/release assets")
...
obj<-readRDS(a$rds);co<-read_any(a$coordinates);cc<-a$`cell-id-col`;co[[cc]]<-as.character(co[[cc]]);cells<-intersect(colnames(obj),co[[cc]]);if(!is.null(a$`expected-observations`)&&length(cells)!=as.integer(a$`expected-observations`))stop("analysis-set observation count mismatch");co<-co[match(cells,get(cc))];xx<-intersect(c("sdimx","x","spatial_x"),names(co))[1];yy<-intersect(c("sdimy","y","spatial_y"),names(co))[1]
...
markers<-read_any(a$markers);stopifnot(all(c("gene","marker_group")%in%names(markers)));if(!is.null(a$level)){if(!"level"%in%names(markers))stop("--level requires a level column in marker input");markers<-markers[as.character(level)==as.character(a$level)]};markers<-unique(markers[,.(gene=as.character(gene),marker_group=as.character(marker_group))]);if(!nrow(markers))stop("No marker rows remain after filtering")
