#!/usr/bin/env Rscript
# Query-only SCE/BANKSY cohort reclustering implementation.
...
cts <- assay(sce,count_assay)[,cells,drop=FALSE]; umi <- Matrix::colSums(cts); zero_ids <- names(umi)[!is.finite(umi)|umi<=0]; fwrite(data.table(cell_id=zero_ids,route="qc_holdout",reason="zero_count_in_selected_assay"),file.path(a$out,"tables","zero_count_observations.tsv"),sep="\t"); if(length(zero_ids)==length(umi))stop("All cohort observations have zero counts"); cts <- cts[,setdiff(colnames(cts),zero_ids),drop=FALSE]; q <- CreateSeuratObject(counts=cts,assay="RNA",project="framework_cohort",min.cells=0,min.features=0)
