...
def main():
    p=argparse.ArgumentParser();p.add_argument("project_root",type=Path);p.add_argument("--context",required=True,type=Path);p.add_argument("--profile",type=Path);a=p.parse_args();root=a.project_root;ctx=json.loads(a.context.read_text());profile=json.loads(a.profile.read_text()) if a.profile else {};all_clusters=read(root/"state/cluster_decision_ledger.tsv");clusters=active_decisions(all_clusters);pools=read(root/"state/pool_registry.tsv");pool_status={r.get("pool_id"):r.get("status") for r in pools};rows=[]
    rare=profile.get("rare_cell_rules",{});priority_terms=[str(x).lower() for x in ctx.get("priority_lineages",[])];multiroute=audit_multiroute(root,ctx,profile);gap_routes={}
    ...
    for r in clusters:
        ...
        elif state=="qc_holdout" and "zero_count" not in evidence and "qc_atlas_review" in gap_routes.get(did,set()): route="depth_matched_atlas_internal_anchor_review";reason="non-zero QC holdout requires a depth-matched atlas/internal-anchor attempt before rescue or retention"
        elif state=="qc_holdout" and not (closed and closure): route="depth_matched_atlas_internal_anchor_review";reason="QC holdout requires calibrated rescue or explicit retention"
        ...
        for name,rule in rare.items():
            if any(re.search(pat,text,re.I) for pat in rule.get("label_patterns",[])):
                if not (validated and closed and closure and artifact_ok and feature_scope=="full_feature" and object_ok): route="strict_rare_cell_validation";reason=f"{name} requires full-feature core/negative/spatial/recluster validation and object-level counting"
                break
        ...
        if route!="none":
            rows.append({"priority":1 if route=="strict_rare_cell_validation" else 2 if "interface" in route else 3,"source_run_id":r.get("source_run_id",""),"source_cluster":r.get("source_cluster",""),"n_observations":n,"current_state":state,"broad_label":broad,"fine_label":fine,"required_route":route,"reason":reason,"target_pool":r.get("target_pool",action),"blocked_until":"validation artifact + state writeback + pool closure"})
    ...
    for gap in multiroute.get("gaps",[]):
        ...
        rows.append({"priority":1 if gap["required_route"]=="strict_rare_cell_review" else 2,"source_run_id":decision.get("source_run_id",""),"source_cluster":decision.get("source_cluster",gap["decision_id"]),"n_observations":gap["n_observations"],"current_state":decision.get("state","multiroute_gap"),"broad_label":decision.get("broad_label",""),"fine_label":decision.get("fine_label",""),"required_route":gap["required_route"],"reason":gap["reason"],"target_pool":decision.get("target_pool",""),"blocked_until":"validated route attempt + atomic writeback"})
    for view in multiroute.get("missing_views",[]):rows.append({"priority":4,"source_run_id":"project","source_cluster":f"__ANNOTATION_VIEW_{view.upper()}__","n_observations":0,"current_state":"project_gate","broad_label":"","fine_label":"","required_route":"build_annotation_views","reason":f"missing validated {view} annotation view","target_pool":"analysis_set","blocked_until":"view census + cell-level fields + artifact validation"})
