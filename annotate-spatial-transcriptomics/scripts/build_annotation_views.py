...
def main() -> int:
    ...
    with lock_path.open("w") as lock:
        ...
        for view in ["strict", "inclusive", "display"]:
            old.append({"view_id":f"{args.sample}_{view}_{args.view_version}","sample_id":args.sample,"view":view,"membership_path":str(membership_path),"membership_sha256":digest,"n_observations":n_analysis,"policy":policy["rule"],"marker_deg_eligible":"true" if view=="strict" else "false","status":"validated","artifact":str((tables/f"{view}_annotation_census.tsv").resolve()),"created_at":now})
