...
def main() -> int:
    ...
    result={"status":"MIGRATED_REQUIRES_REVIEW","source_config_sha256":sha(backup),"created_registries":created,"cell_ledger":str(cell) if cell.exists() else None,"required_next_actions":["audit_multiroute_state","populate route attempts from validated artifacts only","reopen gaps with new decision version","build strict/inclusive/display cell ledger","rerun completion and release audit"]}
