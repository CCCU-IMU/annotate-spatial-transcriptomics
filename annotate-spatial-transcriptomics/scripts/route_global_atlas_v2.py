...
def validate_calibration(
    manifest_path: Path, mapping_path: Path, policy: dict,
    ...
    tier_precision = {
        "moderate_or_higher": float(
            policy.get("moderate_or_higher_target_precision", 0.90)
        ),
        "high": float(policy.get("high_target_precision", 0.95)),
    }
    for row in rows:
        cumulative_tier = str(row.get("cumulative_tier", ""))
        if cumulative_tier not in tier_precision:
            continue
        label = row.get("predicted_label", "")
        n = int(float(row.get("validation_n", 0) or 0))
        precision = float(row.get("validation_precision", 0) or 0)
        ece_text = str(row.get("expected_calibration_error", "")).strip()
        # Calibration files produced before this fix did not report ECE. They
        # remain readable, but cannot claim an ECE PASS; newly generated files
        # are checked independently from precision.
        ece = float(ece_text) if ece_text else None
        ece_eligible = ece is None or ece <= max_ece
        result.setdefault(label, {})[cumulative_tier] = {
            "n": n,
            "precision": precision,
            "expected_calibration_error": ece,
            "ece_status": "legacy_not_available" if ece is None else (
                "PASS" if ece_eligible else "FAIL"
            ),
            "eligible": (
                n >= min_n
                and precision >= tier_precision[cumulative_tier]
                and ece_eligible
            ),
        }
    return result
...
def calibrated_for_mapping_tier(
    calibration: dict[str, dict[str, dict]], source_label: str, tier: str,
    policy: dict,
) -> tuple[bool, dict]:
    class_rows = calibration.get(source_label, {})
    key = "high" if tier == "high" else "moderate_or_higher"
    record = class_rows.get(key)
    if record is None and tier == "high":
        # Backward-compatible fallback for old cumulative tables that emitted
        # only the moderate-or-higher row. A high mapping may use it only when
        # that row independently reaches the configured high precision.
        fallback = class_rows.get("moderate_or_higher")
        if fallback and (
            int(fallback.get("n", 0))
            >= int(policy.get("minimum_heldout_observations_per_return_class", 30))
            and float(fallback.get("precision", 0))
            >= float(policy.get("high_target_precision", 0.95))
            and fallback.get("ece_status") != "FAIL"
        ):
            record = dict(fallback)
            record["calibration_tier_used"] = "moderate_or_higher_high_fallback"
    if record is None:
        return False, {}
    return bool(record.get("eligible", False) or (
        tier == "high"
        and float(record.get("precision", 0))
        >= float(policy.get("high_target_precision", 0.95))
        and int(record.get("n", 0))
        >= int(policy.get("minimum_heldout_observations_per_return_class", 30))
        and record.get("ece_status") != "FAIL"
    )), record
...
def main() -> int:
    ...
    for cell in analysis_ids:
        ...
        class_calibrated, class_calibration = calibrated_for_mapping_tier(
            calibration, source_label, tier, policy
        )
        ...
        rows.append({
            ...
            "atlas_broad": mapped,
            "atlas_tier": tier,
            "atlas_class_validation_n": class_calibration.get("n", ""),
            "atlas_class_validation_precision": class_calibration.get(
                "precision", ""
            ),
            "atlas_class_expected_calibration_error": class_calibration.get(
