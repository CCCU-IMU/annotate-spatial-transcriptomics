...
def main():
    ...
    for f in glob.glob(a.cluster_glob):
        c=pd.read_csv(f,sep=None,engine="python",dtype={a.cell_id_col:str,"cluster":str})
        # Some clustering exports already carry coordinates. Drop those
        # copies and bind the single frozen coordinate source explicitly.
        c=c.drop(columns=[a.x_col,a.y_col],errors="ignore")
        d=c.merge(co,on=a.cell_id_col,validate="one_to_one");res=rtag(f)
