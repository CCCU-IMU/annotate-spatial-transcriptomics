...
membership<-read_any(a$membership);if(!all(c("cell_id","final_broad_label")%in%names(membership)))stop("Membership lacks cell_id/final_broad_label");membership[,cell_id:=as.character(cell_id)];if(anyDuplicated(membership$cell_id)||any(!nzchar(membership$cell_id)))stop("Membership has invalid cell IDs")
...
is_analysis<-meta$cell_id%in%membership$cell_id;broad<-blank_to_na(first_col(meta,c("final_broad_label")));fine<-blank_to_na(first_col(meta,c("final_fine_label")));state<-blank_to_na(first_col(meta,c("state_annotations","final_state_annotation")))
