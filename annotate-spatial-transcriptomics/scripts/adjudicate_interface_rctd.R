...
if (!"rctd_confidence_tier" %in% names(d) || any(!d$rctd_confidence_tier %in% c("extreme", "high", "medium_low"))) stop("tiered RCTD evidence is required")
...
ref <- led[!cell_id %in% ids & state %in% c("defined_fine", "defined_broad_only") & confidence %in% c("high", "medium") & closed_ok & cell_id %in% rownames(all_xy)]
...
d[, post_rctd_validated := rctd_confidence_tier %in% c("extreme", "high") & rctd_program_consistent & cluster_context_consistent & observed_density_spatial_consistent & !plasma_review_candidate]
d[, final_action := "route_to_atlas_internal_anchor_fallback"]
d[plasma_review_candidate == TRUE, final_action := "route_to_plasma_review_pool"]
d[post_rctd_validated & proposed_canonical != "Oocyte", final_action := "validated_broad_return"]
d[post_rctd_validated & proposed_canonical == "Oocyte", final_action := "route_to_strict_oocyte_review_pool"]
...
d[, final_state := fifelse(final_action == "validated_broad_return", "defined_broad_only", "pending_review")]
...
fwrite(d[final_action == "route_to_atlas_internal_anchor_fallback"], file.path(outdir, "tables", "rctd_atlas_fallback_membership.tsv.gz"), sep = "\t")
...
plot_dt <- d[, .(x, y, plot_label = fifelse(final_action == "validated_broad_return", final_broad_label, fifelse(final_action == "route_to_strict_oocyte_review_pool", "Strict Oocyte review", fifelse(final_action == "route_to_plasma_review_pool", "Plasma review", "Atlas/internal-anchor fallback"))))]
pal <- c(Stromal = "#4DAF4A", `Stromal/vascular-associated` = "#377EB8", `Follicular somatic` = "#FF7F00", Theca = "#984EA3", Vascular = "#1F78B4", Immune = "#E31A1C", `Epithelial/mesothelial` = "#A65628", `Strict Oocyte review` = "#FB9A99", `Plasma review` = "#B15928", `Atlas/internal-anchor fallback` = "#D0D0D0")
...
manifest <- list(status = "PASS", n_query = nrow(d), rctd_extreme_n = sum(d$rctd_confidence_tier == "extreme"), rctd_high_n = sum(d$rctd_confidence_tier == "high"), rctd_medium_low_n = sum(d$rctd_confidence_tier == "medium_low"), rctd_fine_return_n = 0, rctd_broad_return_n = sum(d$final_action == "validated_broad_return"), n_atlas_internal_anchor_fallback = sum(d$final_action == "route_to_atlas_internal_anchor_fallback"), n_strict_oocyte_review = sum(d$final_action == "route_to_strict_oocyte_review_pool"), n_plasma_review = sum(d$final_action == "route_to_plasma_review_pool"), strict_oocyte_policy = "RCTD cannot directly define Oocyte; candidate must enter the independent strict rare-cell route", observed_grid_step = grid_step, observed_density_radius = radius, minimum_spatial_fraction = min_spatial_fraction, independent_fine_evidence = FALSE, fine_anchor_eligible = FALSE, warning = "RCTD is low-priority assistance: high/extreme cells can return broad only after independent evidence; medium/low cells must continue to calibrated atlas/internal anchors.")
