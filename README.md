...
面向 Codex Agent 的空间转录组/单细胞转录组迭代式注释 Skill。它不是把一次成功项目的参数和标签复制到新样本，而是让 Agent 根据当前物种、组织、平台、数据复杂度和生物学问题，自主完成：输入发现、聚类选择、大类 cohort 纯度审查、直接回归、按需定向重聚类、terminal residual QC 救回、状态追踪、失败恢复、用户确认以及可审计 HTML 报告。
...
请自主发现输入、选择适合当前数据的聚类强度、投递并监控任务、检查日志、修复失败、维护状态文件并持续迭代。不要复制示例项目的参数或标签。空间数据优先获得可靠大类，亚群只在证据充分时定义。羊卵巢预设先逐簇定义大类，再按大类重聚类；亚簇可直接定义、回归父大类、跨谱系回归目标大类/亚群，或进入 QC holdout。Atlas 只处理所有大类和定向判断结束后的最终残余 QC，不先重聚类 QC。最终只发布一套注释：大类至少中等置信度、亚群仅高置信度。
```
...
输入与表达层审计
  ...
  -> 亚簇判断
     ...
     -> 无法解释：QC holdout
  -> 汇总最终残余 QC holdout，直接以内部大类锚点 + GSE233801/匹配参考做校准 Atlas
     -> 中高置信：仅回归大类
     -> 低置信：保留 QC reject
...
若有同项目或生物学匹配的单细胞数据，应先建立“来源标签 -> 空间候选大类”的可审计 crosswalk，而不是要求两种数据产生完全相同的分类。证据优先级为：当前空间数据全基因锚点与形态 > 同项目/同阶段单细胞参考 > 同物种同组织公共 atlas > 发育期、跨组织或跨物种参考。配对单细胞是优先外部参考，但不能推翻当前空间数据的 marker/anti-marker 和组织学矛盾；其细胞级转移只允许发生在所有大类/定向判断完成后冻结的最终残余 QC 路线内。

只有点图时，可用于完善 marker 组合、anti-marker 和候选命名，不能声称完成细胞级映射。具备 count-level 单细胞对象时，也只有残余 QC 路线才可执行深度匹配、held-out 校准和多通道 broad-only 救回，不能对全对象、已定义大类/亚群或普通生物学群体补跑一次参考映射。默认 transfer ceiling 为大类；参考支持的亚群仍必须在空间 query 内通过全基因证据、重聚类稳定性和形态门。最终空间大类 DEG 和点图必须纳入所有正式回归该大类的空间观测，不能只画直接锚点，也不能混入单细胞参考细胞。
...
- **残余 QC 救回。** 合并初始低质量簇与后续仍不可解释的观测，冻结为最终 `qc_holdout`。不再强制先对 QC 做锚点重聚类；直接对这一完整、唯一 membership 执行 Atlas + 内部锚点 + marker/anti-marker + 观测密度空间证据的联合校准。中等及以上置信度只回归大类，低置信保留为 QC reject。Atlas 不对已定义大类或高置信亚群常规运行。

同批次羊卵巢固定网格只在 active workflow profile、显式 strategy preset、对象/层/marker/坐标审计和 StereoPy conversion provenance 共同通过后生效；文件名出现 `cellbin_PPed` 或 feature 数量较多仅为提示。每个 cohort outcome、direct return 和最终 broad/fine support 都必须通过内容 schema。集合审计进一步验证初始大类、cohort 子结局、direct return、targeted successor、terminal residual QC、Atlas accepted/rejected 和最终 analysis set 逐 cell 互斥闭合；只有文件存在但内容为空不能通过。
...
`audit_annotation_membership_partition.py` 在 completion、主 Agent 质量请求和 release audit 三处强制逐 cell 闭合：initial broad 与 broad cohort query 相等；cohort 结局互斥完备；targeted successor 精确继承来源亚簇；direct return 不重叠；Atlas query 精确等于 terminal residual QC；accepted/rejected 精确划分 query 并与 ledger 写回一致；最终注释唯一覆盖 analysis set。
...
对 Skill 的重要更新必须用新 Agent 做 forward test：只给它原始输入与 Skill，不给预期聚类选择和注释答案；评价其是否能自主完成 cohort 审查、direct return、terminal residual QC、恢复失败、控制易混淆谱系和产出完整报告，而不是是否复现某个旧项目的标签。

## 版本

当前版本：`1.6.1`。发布包的校验和将在 GitHub Release 中提供。
