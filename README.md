...
由 StereoPy `cellbin_PPed` 批量转换得到的 Seurat RDS 只是原始计数输入载体，并不等同于已经完成 SCT 前处理。若同一批样本需要与已验证的 R 流程保持可比，Agent 会读取 `references/seurat-cellbin-preprocessing.md`，从 `Spatial` counts 重新执行统一的 `SCTransform(v2, glmGamPoi)`、PCA 和 cosine Annoy 邻接图，并写出完整 manifest。固定的是批次前处理参数；最终大类分辨率、池子分辨率和生物学标签仍必须针对当前样本自适应判断。
